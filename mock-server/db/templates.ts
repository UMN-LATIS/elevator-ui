import type { Template } from "../../src/types";
import { createBaseTable } from "./baseTable";
import invariant from "tiny-invariant";

// Local copy — mirrors the IDs returned by the mock GET /templates/getFieldTypes endpoint.
// These must stay in sync with that endpoint; they are NOT imported from src/constants.
export const FIELD_TYPE_IDS: Record<string, number> = {
  text: 1,
  "text area": 2,
  select: 3,
  checkbox: 4,
  date: 5,
  "tag list": 6,
  multiselect: 7,
  location: 8,
  upload: 9,
  "related asset": 10,
};

// AdminTemplateSeed extends the asset-editor Template with fields
// needed for the admin template editor. Lives in the mock layer only.
export interface AdminTemplateSeed extends Template {
  createdAt: string;
  modifiedAt: string;
  includeInSearch: boolean;
  indexForSearching: boolean;
  isHidden: boolean;
  recursiveIndexDepth: 0 | 1 | 2;
  templateColor: number;
}

const templateSeeds: AdminTemplateSeed[] = [
  // Minimal template used as the inline child in inline-related-asset e2e tests.
  // Intentionally has no checkbox widget so isBlank behaves predictably.
  {
    templateId: 101,
    templateName: "Inline Child Template",
    showCollection: false,
    showTemplate: false,
    showCollectionPosition: 1,
    showTemplatePosition: 1,
    createdAt: "2024-01-01T00:00:00+00:00",
    modifiedAt: "2024-01-01T00:00:00+00:00",
    includeInSearch: true,
    indexForSearching: true,
    isHidden: false,
    recursiveIndexDepth: 1,
    templateColor: 0,
    widgetArray: [
      {
        widgetId: 10101,
        type: "text",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "title_1",
        label: "Title",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: true,
        required: true,
        searchable: true,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 1,
        templateOrder: 1,
      },
      {
        widgetId: 10102,
        type: "text area",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "description_1",
        label: "Description",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 2,
        templateOrder: 2,
      },
    ],
    collections: { "1": "Default Collection" },
    allowedCollections: { "1": "Default Collection" },
  },
  // Parent template with collapseNestedChildren for related-asset-parent-id e2e tests.
  // The collapsed widget fetches the child asset inline but renders it collapsed.
  {
    templateId: 103,
    templateName: "Collapsed Related Parent Template",
    showCollection: false,
    showTemplate: false,
    showCollectionPosition: 1,
    showTemplatePosition: 1,
    createdAt: "2024-01-01T00:00:00+00:00",
    modifiedAt: "2024-01-01T00:00:00+00:00",
    includeInSearch: true,
    indexForSearching: true,
    isHidden: false,
    recursiveIndexDepth: 1,
    templateColor: 0,
    widgetArray: [
      {
        widgetId: 10301,
        type: "text",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "title_1",
        label: "Title",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: true,
        required: true,
        searchable: true,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 1,
        templateOrder: 1,
      },
      {
        widgetId: 10302,
        type: "related asset",
        allowMultiple: true,
        attemptAutocomplete: false,
        fieldTitle: "collapsedchild_1",
        label: "Collapsed Child",
        tooltip: "",
        fieldData: {
          nestData: true,
          showLabel: false,
          matchAgainst: [1],
          displayInline: true,
          thumbnailView: false,
          defaultTemplate: 1,
          ignoreForDateSearch: false,
          ignoreForDigitalAsset: false,
          collapseNestedChildren: true,
          ignoreForLocationSearch: false,
        },
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 2,
        templateOrder: 2,
      },
    ],
    collections: { "1": "Default Collection" },
    allowedCollections: { "1": "Default Collection" },
  },
  // Parent template with a thumbnailView related asset widget, plus its own
  // uploads, for the description-as-alt-text UX cases.
  {
    templateId: 104,
    templateName: "Thumbnail Related Parent Template",
    showCollection: false,
    showTemplate: false,
    showCollectionPosition: 1,
    showTemplatePosition: 1,
    createdAt: "2024-01-01T00:00:00+00:00",
    modifiedAt: "2024-01-01T00:00:00+00:00",
    includeInSearch: true,
    indexForSearching: true,
    isHidden: false,
    recursiveIndexDepth: 1,
    templateColor: 0,
    widgetArray: [
      {
        widgetId: 10401,
        type: "text",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "title_1",
        label: "Title",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: true,
        required: true,
        searchable: true,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 1,
        templateOrder: 1,
      },
      {
        widgetId: 10402,
        type: "upload",
        allowMultiple: true,
        attemptAutocomplete: false,
        fieldTitle: "upload_1",
        label: "Upload",
        tooltip: "",
        fieldData: {
          extractDate: true,
          forceTiling: false,
          enableDendro: true,
          enableIframe: true,
          enableTiling: true,
          extractLocation: true,
          enableAnnotation: false,
          interactiveTranscript: false,
        },
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 2,
        templateOrder: 2,
      },
      {
        widgetId: 10403,
        type: "related asset",
        allowMultiple: true,
        attemptAutocomplete: false,
        fieldTitle: "thumbnailchildren_1",
        label: "Related Items",
        tooltip: "",
        fieldData: {
          nestData: false,
          showLabel: true,
          matchAgainst: [1],
          displayInline: false,
          thumbnailView: true,
          defaultTemplate: 1,
          ignoreForDateSearch: false,
          ignoreForDigitalAsset: false,
          collapseNestedChildren: false,
          ignoreForLocationSearch: false,
        },
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 3,
        templateOrder: 3,
      },
    ],
    collections: { "1": "Default Collection" },
    allowedCollections: { "1": "Default Collection" },
  },
  // The Nested Record template's related asset widget matches against this
  // same template, so records on it chain into each other and a chain can run
  // deeper than the view page will open inline.
  {
    templateId: 105,
    templateName: "Nested Record",
    showCollection: false,
    showTemplate: false,
    showCollectionPosition: 1,
    showTemplatePosition: 1,
    createdAt: "2024-01-01T00:00:00+00:00",
    modifiedAt: "2024-01-01T00:00:00+00:00",
    includeInSearch: true,
    indexForSearching: true,
    isHidden: false,
    recursiveIndexDepth: 2,
    templateColor: 0,
    widgetArray: [
      {
        widgetId: 10501,
        type: "text",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "title_1",
        label: "Title",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: true,
        required: true,
        searchable: true,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 1,
        templateOrder: 1,
      },
      {
        widgetId: 10502,
        type: "text area",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "description_1",
        label: "Description",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: false,
        required: false,
        searchable: true,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 2,
        templateOrder: 2,
      },
      {
        widgetId: 10503,
        type: "related asset",
        allowMultiple: true,
        attemptAutocomplete: false,
        fieldTitle: "childrecords_1",
        label: "Child Records",
        tooltip: "",
        fieldData: {
          nestData: true,
          showLabel: true,
          matchAgainst: [105],
          displayInline: false,
          thumbnailView: false,
          defaultTemplate: 105,
          ignoreForDateSearch: false,
          ignoreForDigitalAsset: false,
          collapseNestedChildren: false,
          ignoreForLocationSearch: false,
        },
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 3,
        templateOrder: 3,
      },
    ],
    collections: { "1": "Default Collection" },
    allowedCollections: { "1": "Default Collection" },
  },
  // Parent template used in inline-related-asset e2e tests.
  // Has a single inline related asset widget pointing at template 101.
  {
    templateId: 102,
    templateName: "Inline Parent Template",
    showCollection: false,
    showTemplate: false,
    showCollectionPosition: 1,
    showTemplatePosition: 1,
    createdAt: "2024-01-01T00:00:00+00:00",
    modifiedAt: "2024-01-01T00:00:00+00:00",
    includeInSearch: true,
    indexForSearching: true,
    isHidden: false,
    recursiveIndexDepth: 1,
    templateColor: 0,
    widgetArray: [
      {
        widgetId: 10201,
        type: "text",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "title_1",
        label: "Title",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: true,
        required: true,
        searchable: true,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 1,
        templateOrder: 1,
      },
      {
        widgetId: 10202,
        type: "related asset",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "inlinechild_1",
        label: "Inline Child",
        tooltip: "",
        fieldData: {
          nestData: false,
          showLabel: false,
          matchAgainst: [101],
          displayInline: true,
          thumbnailView: false,
          defaultTemplate: 101,
          ignoreForDateSearch: false,
          ignoreForDigitalAsset: false,
          collapseNestedChildren: false,
          ignoreForLocationSearch: false,
        },
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 2,
        templateOrder: 2,
      },
    ],
    collections: { "1": "Default Collection" },
    allowedCollections: { "1": "Default Collection" },
  },
  // Misconfigured template for error boundary testing
  // The select widget is missing `selectGroup` in fieldData which will cause an error
  {
    templateId: 999,
    templateName: "Broken Template (for testing)",
    showCollection: false,
    showTemplate: false,
    showCollectionPosition: 1,
    showTemplatePosition: 1,
    createdAt: "2024-01-10T08:00:00+00:00",
    modifiedAt: "2024-01-10T08:00:00+00:00",
    includeInSearch: true,
    indexForSearching: true,
    isHidden: true,
    recursiveIndexDepth: 1,
    templateColor: 0,
    widgetArray: [
      {
        widgetId: 9991,
        type: "text",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "title_1",
        label: "Title",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: true,
        required: true,
        searchable: true,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 1,
        templateOrder: 1,
      },
      {
        widgetId: 9992,
        type: "select",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "brokenselect_1",
        label: "Broken Select",
        tooltip: "This select widget is missing selectGroup data",
        // Missing fieldData.selectGroup - this will cause an error when rendering
        fieldData: null,
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 2,
        templateOrder: 2,
      },
    ],
    collections: {
      "1": "Default Collection",
    },
    allowedCollections: {
      "1": "Default Collection",
    },
  },
  {
    templateId: 1,
    templateName: "Some Fields",
    showCollection: false,
    showTemplate: false,
    showCollectionPosition: 1,
    showTemplatePosition: 1,
    createdAt: "2024-01-15T10:00:00+00:00",
    modifiedAt: "2024-03-20T14:30:00+00:00",
    includeInSearch: true,
    indexForSearching: true,
    isHidden: false,
    recursiveIndexDepth: 1,
    templateColor: 0,
    widgetArray: [
      {
        widgetId: 19,
        type: "text",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "title_1",
        label: "Title",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: true,
        required: true,
        searchable: true,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 1,
        templateOrder: 1,
      },
      {
        widgetId: 20,
        type: "checkbox",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "checkbox_1",
        label: "Checkbox",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 2,
        templateOrder: 2,
      },
      {
        widgetId: 21,
        type: "upload",
        allowMultiple: true,
        attemptAutocomplete: false,
        fieldTitle: "upload_1",
        label: "Upload",
        tooltip: "",
        fieldData: {
          extractDate: true,
          forceTiling: false,
          enableDendro: true,
          enableIframe: true,
          enableTiling: true,
          extractLocation: true,
          enableAnnotation: false,
          interactiveTranscript: false,
        },
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 3,
        templateOrder: 3,
      },
      {
        widgetId: 22,
        type: "date",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "date_1",
        label: "Date",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 4,
        templateOrder: 4,
      },
      {
        widgetId: 23,
        type: "location",
        allowMultiple: true,
        attemptAutocomplete: false,
        fieldTitle: "location_1",
        label: "Location",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 5,
        templateOrder: 5,
      },
    ],
    collections: {
      "1": "Default Collection",
    },
    allowedCollections: {
      "1": "Default Collection",
    },
  },
  {
    templateId: 68,
    templateName: "All Fields Test",
    showCollection: true,
    showTemplate: true,
    showCollectionPosition: 0,
    showTemplatePosition: 0,
    createdAt: "2023-11-01T09:00:00+00:00",
    modifiedAt: "2025-12-10T16:45:00+00:00",
    includeInSearch: true,
    indexForSearching: true,
    isHidden: false,
    recursiveIndexDepth: 2,
    templateColor: 3,
    widgetArray: [
      {
        widgetId: 45306,
        type: "text",
        allowMultiple: true,
        attemptAutocomplete: false,
        fieldTitle: "title_1",
        label: "Title",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: true,
        required: true,
        searchable: true,
        directSearch: false,
        clickToSearch: true,
        clickToSearchType: 1,
        viewOrder: 1,
        templateOrder: 1,
      },
      {
        widgetId: 45307,
        type: "checkbox",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "checkbox_1",
        label: "Checkbox",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 2,
        templateOrder: 2,
      },
      {
        widgetId: 45308,
        type: "date",
        allowMultiple: true,
        attemptAutocomplete: false,
        fieldTitle: "creation_1",
        label: "Creation",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 3,
        templateOrder: 3,
      },
      {
        widgetId: 45309,
        type: "location",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "location_1",
        label: "Location",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 4,
        templateOrder: 4,
      },
      {
        widgetId: 45310,
        type: "multiselect",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "cascadeselect_1",
        label: "Cascade Select",
        tooltip: "",
        fieldData: {
          country: {
            usa: {
              "State or Province": {
                minnesota: {
                  city: {
                    mankato: {
                      neighborhood: ["campus", "downtown"],
                    },
                    minneapolis: {
                      neighborhood: ["uptown", "downtown"],
                    },
                    "St. Paul": {
                      neighborhood: ["Summit Hill", "frogtown"],
                    },
                  },
                },
                wisconsin: {
                  city: ["madison", "milwaukee"],
                },
              },
            },
            canada: {
              "State or Province": {
                quebec: {
                  city: ["montreal"],
                },
                alberta: {
                  city: ["fakeville", "faketown"],
                },
              },
            },
          },
        },
        display: true,
        displayInPreview: false,
        required: false,
        searchable: true,
        directSearch: true,
        clickToSearch: true,
        clickToSearchType: 1,
        viewOrder: 5,
        templateOrder: 5,
      },
      {
        widgetId: 45311,
        type: "related asset",
        allowMultiple: true,
        attemptAutocomplete: true,
        fieldTitle: "relatedstuff_1",
        label: "Related Stuff",
        tooltip: "",
        fieldData: {
          nestData: true,
          showLabel: true,
          matchAgainst: [68, 45],
          displayInline: false,
          thumbnailView: false,
          defaultTemplate: 68,
          ignoreForDateSearch: false,
          ignoreForDigitalAsset: false,
          collapseNestedChildren: false,
          ignoreForLocationSearch: false,
        },
        display: true,
        displayInPreview: true,
        required: false,
        searchable: true,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 6,
        templateOrder: 6,
      },
      {
        widgetId: 45312,
        type: "select",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "globalsearch_1",
        label: "Global Search",
        tooltip: "",
        fieldData: {
          multiSelect: false,
          selectGroup: ["", "I Agree"],
        },
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: true,
        clickToSearchType: 0,
        viewOrder: 7,
        templateOrder: 7,
      },
      {
        widgetId: 45313,
        type: "tag list",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "sometags_1",
        label: "Some tags",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 8,
        templateOrder: 8,
      },
      {
        widgetId: 45317,
        type: "tag list",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "clickabletags_1",
        label: "Clickable tags",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: false,
        required: false,
        searchable: true,
        directSearch: true,
        clickToSearch: true,
        clickToSearchType: 1,
        viewOrder: 9,
        templateOrder: 9,
      },
      {
        widgetId: 45314,
        type: "text area",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "bigtext_1",
        label: "Big Text",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 10,
        templateOrder: 10,
      },
      {
        widgetId: 45315,
        type: "upload",
        allowMultiple: true,
        attemptAutocomplete: false,
        fieldTitle: "upload_1",
        label: "Upload",
        tooltip: "",
        fieldData: {
          extractDate: true,
          forceTiling: false,
          enableDendro: false,
          enableIframe: false,
          enableTiling: true,
          extractLocation: true,
          enableAnnotation: false,
          interactiveTranscript: false,
        },
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 11,
        templateOrder: 11,
      },
      {
        widgetId: 45316,
        type: "related asset",
        allowMultiple: true,
        attemptAutocomplete: false,
        fieldTitle: "collapsedjoinrecord_1",
        label: "Collapsed Join Record",
        tooltip: "",
        fieldData: {
          nestData: true,
          showLabel: true,
          matchAgainst: [1],
          displayInline: true,
          thumbnailView: false,
          defaultTemplate: 1,
          ignoreForDateSearch: false,
          ignoreForDigitalAsset: false,
          collapseNestedChildren: false,
          ignoreForLocationSearch: false,
        },
        display: true,
        displayInPreview: true,
        required: false,
        searchable: true,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 12,
        templateOrder: 12,
      },
    ],
    collections: {
      "1": "Default Collection",
    },
    allowedCollections: {
      "1": "Default Collection",
    },
  },
  {
    templateId: 3,
    templateName: "Multiple Upload Widgets",
    showCollection: false,
    showTemplate: false,
    showCollectionPosition: 1,
    showTemplatePosition: 1,
    createdAt: "2024-02-01T12:00:00+00:00",
    modifiedAt: "2024-06-15T09:20:00+00:00",
    includeInSearch: true,
    indexForSearching: true,
    isHidden: false,
    recursiveIndexDepth: 1,
    templateColor: 0,
    widgetArray: [
      {
        widgetId: 3001,
        type: "text",
        allowMultiple: true,
        attemptAutocomplete: false,
        fieldTitle: "title_1",
        label: "Title",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: true,
        required: true,
        searchable: true,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 1,
        templateOrder: 1,
      },
      {
        widgetId: 3002,
        type: "upload",
        allowMultiple: true,
        attemptAutocomplete: false,
        fieldTitle: "upload_1",
        label: "Multiple Uploads",
        tooltip: "",
        fieldData: {
          extractDate: true,
          forceTiling: false,
          enableDendro: true,
          enableIframe: true,
          enableTiling: true,
          extractLocation: true,
          enableAnnotation: false,
          interactiveTranscript: false,
        },
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 3,
        templateOrder: 3,
      },
      {
        widgetId: 3003,
        type: "upload",
        // max of 1 upload
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "upload_2",
        label: "Single Upload",
        tooltip: "",
        fieldData: {
          extractDate: true,
          forceTiling: false,
          enableDendro: true,
          enableIframe: true,
          enableTiling: true,
          extractLocation: true,
          enableAnnotation: false,
          interactiveTranscript: false,
        },
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 3,
        templateOrder: 3,
      },
      {
        widgetId: 22,
        type: "date",
        allowMultiple: true,
        attemptAutocomplete: false,
        fieldTitle: "date_1",
        label: "Date",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 4,
        templateOrder: 4,
      },
      {
        widgetId: 23,
        type: "location",
        allowMultiple: true,
        attemptAutocomplete: false,
        fieldTitle: "location_1",
        label: "Location",
        tooltip: "",
        fieldData: [],
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 5,
        templateOrder: 5,
      },
    ],
    collections: {
      "1": "Default Collection",
    },
    allowedCollections: {
      "1": "Default Collection",
    },
  },
  {
    templateId: 4,
    templateName: "All Fields with Autocomplete",
    showCollection: false,
    showTemplate: false,
    showCollectionPosition: 1,
    showTemplatePosition: 1,
    createdAt: "2024-03-05T14:00:00+00:00",
    modifiedAt: "2025-01-22T11:10:00+00:00",
    includeInSearch: true,
    indexForSearching: true,
    isHidden: false,
    recursiveIndexDepth: 1,
    templateColor: 0,
    widgetArray: [
      {
        widgetId: 4001,
        type: "text",
        allowMultiple: true,
        attemptAutocomplete: true,
        fieldTitle: "title_1",
        label: "Title",
        tooltip: "Title field with autocomplete suggestions",
        fieldData: [],
        display: true,
        displayInPreview: true,
        required: true,
        searchable: true,
        directSearch: false,
        clickToSearch: true,
        clickToSearchType: 1,
        viewOrder: 1,
        templateOrder: 1,
      },
      {
        widgetId: 4002,
        type: "text",
        allowMultiple: false,
        attemptAutocomplete: true,
        fieldTitle: "subtitle_1",
        label: "Subtitle",
        tooltip: "Subtitle with autocomplete",
        fieldData: [],
        display: true,
        displayInPreview: false,
        required: false,
        searchable: true,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 2,
        templateOrder: 2,
      },
      {
        widgetId: 4003,
        type: "text area",
        allowMultiple: false,
        attemptAutocomplete: true,
        fieldTitle: "description_1",
        label: "Description",
        tooltip: "Description with text suggestions",
        fieldData: [],
        display: true,
        displayInPreview: false,
        required: false,
        searchable: true,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 3,
        templateOrder: 3,
      },
      {
        widgetId: 4004,
        type: "tag list",
        allowMultiple: false,
        attemptAutocomplete: true,
        fieldTitle: "keywords_1",
        label: "Keywords",
        tooltip: "Keywords with tag suggestions",
        fieldData: [],
        display: true,
        displayInPreview: false,
        required: false,
        searchable: true,
        directSearch: true,
        clickToSearch: true,
        clickToSearchType: 1,
        viewOrder: 4,
        templateOrder: 4,
      },
      {
        widgetId: 4005,
        type: "related asset",
        allowMultiple: true,
        attemptAutocomplete: true,
        fieldTitle: "relatedassets_1",
        label: "Related Assets",
        tooltip: "Related assets with search autocomplete",
        fieldData: {
          nestData: true,
          showLabel: true,
          matchAgainst: [1, 68, 3, 4],
          displayInline: false,
          thumbnailView: false,
          defaultTemplate: 68,
          ignoreForDateSearch: false,
          ignoreForDigitalAsset: false,
          collapseNestedChildren: false,
          ignoreForLocationSearch: false,
        },
        display: true,
        displayInPreview: true,
        required: false,
        searchable: true,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 5,
        templateOrder: 5,
      },
      {
        widgetId: 4006,
        type: "checkbox",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "published_1",
        label: "Published",
        tooltip: "Publication status",
        fieldData: [],
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 6,
        templateOrder: 6,
      },
      {
        widgetId: 4007,
        type: "date",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "creation_1",
        label: "Creation Date",
        tooltip: "Date of creation",
        fieldData: [],
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 7,
        templateOrder: 7,
      },
      {
        widgetId: 4008,
        type: "location",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "location_1",
        label: "Location",
        tooltip: "Geographic location",
        fieldData: [],
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 8,
        templateOrder: 8,
      },
      {
        widgetId: 4009,
        type: "select",
        allowMultiple: false,
        attemptAutocomplete: false,
        fieldTitle: "category_1",
        label: "Category",
        tooltip: "Asset category",
        fieldData: {
          multiSelect: false,
          selectGroup: [
            "",
            "Academic",
            "Research",
            "Administrative",
            "Cultural",
          ],
        },
        display: true,
        displayInPreview: false,
        required: false,
        searchable: true,
        directSearch: true,
        clickToSearch: true,
        clickToSearchType: 1,
        viewOrder: 9,
        templateOrder: 9,
      },
      {
        widgetId: 4010,
        type: "upload",
        allowMultiple: true,
        attemptAutocomplete: false,
        fieldTitle: "files_1",
        label: "Files",
        tooltip: "Upload files",
        fieldData: {
          extractDate: true,
          forceTiling: false,
          enableDendro: false,
          enableIframe: false,
          enableTiling: true,
          extractLocation: true,
          enableAnnotation: false,
          interactiveTranscript: false,
        },
        display: true,
        displayInPreview: false,
        required: false,
        searchable: false,
        directSearch: false,
        clickToSearch: false,
        clickToSearchType: 1,
        viewOrder: 10,
        templateOrder: 10,
      },
    ],
    collections: {
      "1": "Default Collection",
    },
    allowedCollections: {
      "1": "Default Collection",
    },
  },
];

interface ComparisonResult {
  type: string; // "missing"?
  label: string; // "Title", "Checkbox", etc.
}

type WidgetDefKey = string;
type TemplateComparison = Record<WidgetDefKey, ComparisonResult>;

export function createTemplatesTable() {
  const baseTable = createBaseTable(
    (template: AdminTemplateSeed) => template.templateId,
    templateSeeds
  );

  return {
    ...baseTable,

    create(data: Omit<AdminTemplateSeed, "templateId">): AdminTemplateSeed {
      const id =
        Math.max(0, ...baseTable.getAllKeys().map((k) => Number(k))) + 1;
      const created: AdminTemplateSeed = { ...data, templateId: id };
      baseTable.set(id, created);
      return created;
    },

    update(
      id: number,
      data: Omit<AdminTemplateSeed, "templateId">
    ): AdminTemplateSeed | undefined {
      const existing = baseTable.get(id);
      if (!existing) return undefined;
      const updated: AdminTemplateSeed = { ...data, templateId: id };
      baseTable.set(id, updated);
      return updated;
    },

    compare(
      sourceTemplate: Template,
      destTemplate: Template
    ): TemplateComparison {
      // check for which fields are missing in b that exist in a
      const sourceFields = sourceTemplate.widgetArray.map((w) => w.fieldTitle);

      const comparison: TemplateComparison = sourceFields.reduce(
        (acc, field) => {
          const sourceWidget = sourceTemplate.widgetArray.find(
            (w) => w.fieldTitle === field
          );
          invariant(sourceWidget, `Widget with fieldTitle ${field} not found`);

          const destWidget = destTemplate.widgetArray.find(
            (w) => w.fieldTitle === field
          );

          if (!destWidget) {
            return {
              ...acc,
              [field]: { type: "missing", label: sourceWidget.label },
            };
          }

          if (destWidget.type !== sourceWidget.type) {
            return {
              ...acc,
              [field]: { type: "type mismatch", label: sourceWidget.label },
            };
          }

          return acc;
        },
        {}
      );

      return comparison;
    },
  };
}

export type TemplatesTable = ReturnType<typeof createTemplatesTable>;
