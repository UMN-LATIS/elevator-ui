...
/**
 * Returns a getter for the saves recorded after this call, so a test can
 * assert only about saves the editor sends from here on. Anchoring on the
 * held save instead would also catch the queue's legitimate trailing save
 * for the asset the user was still editing at the time.
 */
function trackNewSaves(saves: RecordedSave[]): () => RecordedSave[] {
  const trackedFromLength = saves.length;
  return () => saves.slice(trackedFromLength);
}
...
test.describe("Asset editor concurrency", () => {
  ...
  test("starting a new draft while the previous asset is still saving does not overwrite the previous asset", async ({
    ...
    const { saves, releaseHeldSave } = await interceptSaves(page, ASSET_1_ID);
  ...
  test("a save in flight does not land on the different asset the user moved to", async ({
    ...
    const { saves, releaseHeldSave } = await interceptSaves(page, newAssetId);
  ...
  test("a create in flight for one draft does not make the next draft adopt its asset", async ({
    ...
    await startDraft(page);
    const savesFromSecondDraftOnward = trackNewSaves(saves);
