<template>
    <template v-if="haveLocation">
        <span><a href="#" @click.prevent="show = true">{{ locationLabel }}</a></span>
...
const locationLabel = computed(() => {
    let label = "";
    if (props.locationContent.locationLabel && props.locationContent.locationLabel.length > 0) {
        label = props.locationContent.locationLabel;
    }
    else {
        if (props.locationContent.address && props.locationContent.address.length > 0) {
...
const haveLocation = computed(() => {
    return props.locationContent.loc.coordinates[0] !== 0 && props.locationContent.loc.coordinates[1] !== 0;
})
